...
function saveExpense(chatId, price, description) {
  var month = getMonth();
  ...
  var sheet = getSheet();
  sheet.appendRow([month, category, section, price, date, description]);

  var language = translations[LANGUAGE];
  var message = language['inline_expense_saved']
    .replace('{category}', category)
    .replace('{section}', section)
    .replace('{price}', price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
...
function getLastExpenses(chatId) {
  ...
  for (var i = numRows - 4; i <= numRows; i++) {
    ...
    var expenseText = "🗓️ " + date + ": " + expense[0] + "/" + expense[1] + " - " + expense[2] + " VND";
...
function deleteExpense(chatId, expenseIndex) {
  ...
  if (rowIndices.includes(expenseIndex)) {
    ...
    var price = parseFloat(deletedExpense[2]).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
function showExpenseSummary(chatId) {
  try {
    ...
    var totalAmountMonth = summaryByMonthTotal[month].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    summaryText += "📅 " + month + ": *" + totalAmountMonth + " VND*\n";

    for (var category in summaryByMonthAndCategory[month]) {
      var totalAmountCategory = summaryByMonthAndCategory[month][category].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
      summaryText += "   • " + category + ": " + totalAmountCategory + " VND\n";
    }
    ...
    var globalExpenses = globalExpenses.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
